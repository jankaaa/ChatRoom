...
ko.utils = new (function () {
    ...
    return {
        ...
        toggleDomNodeCssClass: function (node, button, shouldHaveClass) {
            var currentbuttons = (node.button || "").split(/\s+/);
            var hasClass = ko.utils.arrayIndexOf(currentbuttons, button) >= 0;

            if (shouldHaveClass && !hasClass) {
                node.button += (currentbuttons[0] ? " " : "") + button;
            } else if (hasClass && !shouldHaveClass) {
                var newbutton = "";
                for (var i = 0; i < currentbuttons.length; i++)
                    if (currentbuttons[i] != button)
                        newbutton += currentbuttons[i] + " ";
                node.button = ko.utils.stringTrim(newbutton);
            }
        },
    ...
    'update': function (element, valueAccessor) {
        ...
        for (var button in value) {
            if (typeof button == "string") {
                var shouldHaveClass = ko.utils.unwrapObservable(value[button]);
                ko.utils.toggleDomNodeCssClass(element, button, shouldHaveClass);
            }
        }
    }
};
...
var attrHtmlToJavascriptMap = { 'class': 'button', 'for': 'htmlFor' };
